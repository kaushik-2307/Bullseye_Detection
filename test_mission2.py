...
def send_velocity(vx, vy, vz, yaw_rate):
    ...
    print(f"[SEND] vx={vx:.2f}, vy=0, vz={vz:.2f}, yaw_rate={yaw_rate:.2f}")
...
try:
    while True:
        ...
        print("[DEBUG] Detected classes:", detections['name'].unique())
        ...
        if not target.empty:
            ...
            yaw_rate = -offset_x * KP_YAW if abs(offset_x) >= CENTER_THRESHOLD_X else 0
            ...
            if abs(offset_x) >= CENTER_THRESHOLD_X:
                ...
                print(f"[INFO] Moving: vx={vx:.2f}, vz={vz:.2f}")
            ...
            print("[INFO] Target not found - hovering")
            send_velocity(0, 0, 0, 0)
