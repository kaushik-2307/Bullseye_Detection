...
# -------------------------
print(f"Connecting to autopilot on {MAVLINK_CONNECTION}...")
try:
    master = mavutil.mavlink_connection(MAVLINK_CONNECTION, baud=BAUD_RATE)
    master.wait_heartbeat(timeout=10)
    print(f"Heartbeat from system (system {master.target_system} component {master.target_component})")
except Exception as e:
    print(f"WARNING: Failed to connect to autopilot: {e}")
    print("Continuing without MAVLink - mission control disabled")
    master = None
...
def pause_mission():
    ...
    if master is None:
        print("MAVLink not connected - cannot pause mission")
        return False
    try:
        ...
        print(f"[{iso_timestamp()}] Mission PAUSE command sent")
        return True
    except Exception as e:
...
def resume_mission():
    ...
    if master is None:
        print("MAVLink not connected - cannot resume mission")
        return False
    try:
        master.mav.command_long_send(
            master.target_system,
        ...
        print(f"[{iso_timestamp()}] Mission RESUME command sent")
        return True
    except Exception as e:
...
def set_servo(channel, pwm):
    ...
    if master is None:
        print("MAVLink not connected - cannot set servo")
        return False
    try:
        master.mav.command_long_send(
            master.target_system,
        ...
        print(f"[{iso_timestamp()}] Servo {channel} set to {pwm} PWM")
        return True
    except Exception as e:
...
def handle_flood_detection():
    """Execute the flood detection response sequence"""
    global flood_handling
    flood_handling = True

    print("\n" + "="*60)
    print("DISASTER-FLOOD DETECTED! Executing response sequence...")
    print("="*60)
    ...
    print(f"Waiting {SERVO_DELAY_SEC} seconds...")
    ...
    print("Response sequence completed. Mission resumed.")
    print("="*60 + "\n")
...
try:
    ...
    print("Camera optimized for 1 m/s survey at 8m altitude")
    print("Note: Change ExposureTime to 1000 and AnalogueGain to 4.0 for 2 m/s speed")
except Exception as e:
    print(f"Warning: Could not set all camera controls: {e}")
...
print("="*60)
print("YOLOv8 TFLite — Raspberry Pi 5 + Picamera2 + Mission Control")
print("Disaster-flood detection active")
print("Controls: + - z reset | q to quit")
print("="*60)
    ...
    while True:
        ...
        for d in dets:
            ...
            if class_name in DISASTER_CLASS_NAMES:
                ...
                print(f"\n!!! DISASTER-FLOOD DETECTED at {iso_timestamp()} with confidence {d['conf']:.2f} !!!")
                break
